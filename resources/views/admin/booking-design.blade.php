...
<body class="min-h-screen bg-slate-950 text-stone-50">
    <script>
        window.clientPortalDesignProps = @js([
            ...
            'routes' => [
                'back' => route('admin.bookings.show', $booking),
            ],
            'fonts' => $fonts,
        ]);
    </script>
    <div class="mx-auto max-w-7xl px-6 py-8">
        <div class="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-white/10 bg-white/[0.04] px-5 py-4">
            ...
            <a href="{{ route('admin.bookings.show', $booking) }}" class="rounded-2xl border border-white/10 px-4 py-2 text-sm font-medium text-stone-200 transition hover:bg-white/5">Back to booking</a>
        </div>
