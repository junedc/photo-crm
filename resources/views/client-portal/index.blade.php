...
<body class="min-h-screen bg-slate-950 text-stone-50">
    <div class="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
        ...
        <div class="mb-8 flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-white/10 bg-white/[0.04] p-5 sm:p-6">
            <div class="space-y-2">
                <p class="text-[11px] uppercase tracking-[0.35em] text-cyan-200">Client Portal</p>
                <h1 class="text-2xl font-semibold tracking-tight text-white sm:text-3xl">{{ $customerName ?: $customerEmail }}</h1>
                <p class="text-sm leading-6 text-stone-300">Review your bookings, design drafts, and any client tasks from {{ $tenant->name }}.</p>
            </div>
                        ...
                        <article class="rounded-3xl border border-white/10 bg-white/[0.04] p-4 sm:p-6">
                            ...
                            <div class="flex flex-wrap items-start justify-between gap-3">
                                <div>
                                    ...
                                    <h3 class="mt-1 text-lg font-semibold text-white sm:text-xl">{{ $booking->package?->name ?? 'Custom booking' }}</h3>
                                </div>
                            ...
                            <div class="mt-5 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
                                <a href="{{ route('client.portal.design', $booking) }}" class="rounded-2xl border border-cyan-300/30 px-4 py-2 text-center text-sm font-medium text-cyan-100 transition hover:bg-cyan-300/10 sm:text-left">
                                    {{ $booking->clientPortalDesign ? 'Edit design draft' : 'Create design draft' }}
                                </a>
                                @if ($booking->clientPortalDesign?->last_saved_at)
                                    <span class="inline-flex items-center justify-center rounded-2xl border border-white/10 px-4 py-2 text-xs text-stone-400 sm:justify-start">
                            ...
                            @if ($clientTasks->isNotEmpty())
                                <div class="mt-6 rounded-3xl border border-white/10 bg-slate-950/40 p-4 sm:p-5">
